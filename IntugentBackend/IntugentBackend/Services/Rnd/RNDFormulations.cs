...
namespace IntugentBackend.Services.Rnd
{
    public class RNDFormulations
    {
        ...
        public DataTable dtPO = new DataTable(), dtIso = new DataTable();
        ...
        // Make sure that the first material in the PO and Iso list declared in initializing the page have all the following variables
        ...
        {
            int ico, ifo;
            double dsumPbw, dsumPbwPO, dsumOH, dsumOHPO, dsumFunc, dsumFunc1, dsumBlowingAg, dsumBlowingAg1, dsumWater, dsumSurfac, dsumCatalyst;
            ...
            try
            {
                for (ifo = 0; ifo < Forms.nForm; ifo++)
                {
                    ...
                    for (ico = 0; ico < Forms.POMats.Count; ico++)
                    {
                        if (string.IsNullOrEmpty(Forms.POMats[ico].MatName))
                        {
                            ...
                            {
                                sMsg = "You must choose the material for the row " + (ico + 1) + " of the Polyol Side Materials table";
                                return;
                            }
                            else continue;
                        }
                        ...
                        {
                            dsumPbw += Forms.FormAr[ifo].POMatPbw[ico];
                            dsumOH += Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].MatOHNum;
                            if (Forms.POMats[ico].MatType.Contains("Polyol"))
                            {
                                dsumPbwPO += Forms.FormAr[ifo].POMatPbw[ico];
                                dsumOHPO += Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].MatOHNum;
                            }
                            if (Forms.POMats[ico].MatFunc > 0)
                            {
                                dsumFunc += Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].MatOHNum * Forms.POMats[ico].MatFunc;
                                dsumFunc1 += Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].MatOHNum;
                            }
                            if (Forms.POMats[ico].GassToLiqWtRatio > 0.0)
                            {
                                dsumBlowingAg += Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].GassToLiqWtRatio;
                                dsumBlowingAg1 += Forms.FormAr[ifo].POMatPbw[ico];
                            }
                            if (Forms.POMats[ico].MatType.Contains("Catalyst")) dsumCatalyst += Forms.FormAr[ifo].POMatPbw[ico];
                            if (Forms.POMats[ico].MatType.Contains("Surfactant")) dsumSurfac += Forms.FormAr[ifo].POMatPbw[ico];
                            if (Forms.POMats[ico].ID == 74) dsumWater += Forms.FormAr[ifo].POMatPbw[ico];
                        }
                    }

                    Forms.FormAr[ifo].OHNumPOSide = dsumPbw > 0 ? dsumOH / dsumPbw : 0;
                    Forms.FormAr[ifo].OHNumPolyol = dsumPbwPO > 0 ? dsumOHPO / dsumPbwPO : 0;
                    Forms.FormAr[ifo].FuncAvPOSide = dsumFunc1 > 0 ? dsumFunc / dsumFunc1 : 0;
                    ...
                    Forms.FormAr[ifo].BlowingAgentWtFr = dsumBlowingAg;   // Total wt of blowing agent in PO side. Normalized later
                    Forms.FormAr[ifo].SurfactWtFr = dsumSurfac;          // Total wt. normalized later
                ...
                for (ifo = 0; ifo < Forms.nForm; ifo++)
                {
                    ...
                    else
                    {
                        ...
                        Forms.FormAr[ifo].IsocyanuratePbw = dsumPbw > 0
                            ? 100.0 * (1.0 - 100.0 / Forms.FormAr[ifo].NcoIndex) * Forms.FormAr[ifo].TotalPbwIsoSide / dsumPbw
                            : 0;
                ...
                for (ifo = 0; ifo < Forms.nForm; ifo++)
                {
                    ...
                    for (ico = 0; ico < Forms.POMats.Count; ico++)
                    {
                        ...
                        {
                            temp2 += Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].GassToLiqWtRatio; // gas weight
                            temp3 += 413.0 / 273.0 * 22.41 * Forms.FormAr[ifo].POMatPbw[ico] * Forms.POMats[ico].GassToLiqWtRatio / Forms.POMats[ico].GassMolWt; // STP Gas vol = 22.41 m3/kg-mole
                        }
                    }
                    ...
                    {
                        Forms.FormAr[ifo].BlowingAgentWtFr = Forms.FormAr[ifo].BlowingAgentWtFr / temp1 * 100.0;
                        Forms.FormAr[ifo].CatalystWtFr = Forms.FormAr[ifo].CatalystWtFr / temp1 * 100.0;
                        Forms.FormAr[ifo].SurfactWtFr = Forms.FormAr[ifo].SurfactWtFr / temp1 * 100.0;
                        Forms.FormAr[ifo].WaterWtFr = Forms.FormAr[ifo].WaterWtFr / temp1 * 100.0;
                        Forms.FormAr[ifo].BlowingAgentWtFr1 = Forms.FormAr[ifo].BlowingAgentWtFr1 / temp1 * 100.0;
                    }
                }
            }
            catch (Exception ex)
        ...
        public void ReadDataset()
        {
            ...
            if (RNDHome.drS == null) return;

            nRows = RNDHome.drS["PORows"] != DBNull.Value ? (int)RNDHome.drS["PORows"] : 3;
            ...
            // Adjust for the user assigned OH numbers
            ...
            for (int i = 0; i < RNDHome.dtF.Rows.Count; i++)
            {
                ...
                {
                    Forms.FormAr[i].POMatPbw = System.Text.Json.JsonSerializer.Deserialize<double[]>((string)RNDHome.dtF.Rows[i]["POPbws"]) ?? Forms.FormAr[i].POMatPbw;
                    switch (i)
                    {
                        case 0:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[0].POMatPbw[j] > 0 ? Forms.FormAr[0].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw1 = stmp; }
                            break;
                        case 1:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[1].POMatPbw[j] > 0 ? Forms.FormAr[1].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw2 = stmp; }
                            break;
                        case 2:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[2].POMatPbw[j] > 0 ? Forms.FormAr[2].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw3 = stmp; }
                            break;
                        case 3:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[3].POMatPbw[j] > 0 ? Forms.FormAr[3].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw4 = stmp; }
                            break;
                        case 4:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[4].POMatPbw[j] > 0 ? Forms.FormAr[4].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw5 = stmp; }
                            break;
                        case 5:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[5].POMatPbw[j] > 0 ? Forms.FormAr[5].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw6 = stmp; }
                            break;
                        case 6:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[6].POMatPbw[j] > 0 ? Forms.FormAr[6].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw7 = stmp; }
                            break;
                        case 7:
                            for (j = 0; j < nRows; j++) { stmp = Forms.FormAr[7].POMatPbw[j] > 0 ? Forms.FormAr[7].POMatPbw[j].ToString(sPOf) : string.Empty; Forms.POMats[j].Pbw8 = stmp; }
                            break;
                    }
                }
            }
            ...
            for (int i = 0; i < RNDHome.dtF.Rows.Count; i++)
            {
                Forms.FormAr[i].NcoIndex = RNDHome.dtF.Rows[i]["NCOIndex"] != DBNull.Value
                    ? (double)RNDHome.dtF.Rows[i]["NCOIndex"]
                    : 270;
            }

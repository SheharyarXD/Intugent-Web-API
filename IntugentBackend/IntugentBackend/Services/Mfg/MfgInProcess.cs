...
namespace IntugentBackend.Services.Mfg
{
    public class MfgInProcess
    {
        ...
        public DataRow dr, drFG;
        public bool bDataSetChanged = false;
        public string sNull = " ", sDateNullFormat = " ";
        public string sTimeFormat = "MM/dd/yyyy - hh:mm:tt";
        public const string sDef = "", sOr = "0.000";

        Double[] ThicknessIP = new double[17];
        Double[] CompressiveIP = new double[6];

        public Cbfile Cbfile;

        public MfgInProcess(Cbfile cbfile)
        {
            Cbfile = cbfile ?? throw new ArgumentNullException(nameof(cbfile));
            GetDataSet();
        }
        ...
        public void GetDataSet()
        {
            ...
            try
            {
                if (Cbfile == null)
                {
                    System.Diagnostics.Debug.WriteLine("Cbfile is null in GetDataSet");
                    return;
                }
                ...
                sSqlQuery = "Select * from [In Process Identify] where ID4ALL=" + Cbfile.iIDMfg.ToString();
                da = new SqlDataAdapter(sSqlQuery, Cbfile.conAZ);
        ...
        public string SetDoubleField(string value, string sField)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"SetDoubleField error: {ex.Message}");
                return null;
            }
        }
        ...
        public string SetDoubleTextField(string sField, string sForm = sDef)
        {
            ...
            try
            {
                if (dr != null && !(dr[sField] == DBNull.Value))
                    s = ((double)dr[sField]).ToString(sForm);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"SetDoubleTextField error for field {sField}: {ex.Message}");
                s = string.Empty;
            }
            return s;
        }
        ...
        public string SetIntTextField(string sField)
        {
            ...
            {
                if (dr != null && !(dr[sField] == DBNull.Value))
                    s = ((int)dr[sField]).ToString();
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"SetIntTextField error for field {sField}: {ex.Message}");
                s = string.Empty;
            }
            return s;
        }

        // Updated: Removed TextBox parameter, now takes string value directly
        public void SetDoubleFieldValue(string value, string sField)
        {
            ...
            try
            {
                ...
                {
                    if (double.TryParse(value, out dtmp))
                    {
                        dr[sField] = dtmp;
                    }
                    else
                    {
                        System.Diagnostics.Debug.WriteLine($"Improper Value for {sField}. Value '{value}' is not accepted.");
                    }
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"SetDoubleFieldValue error: {ex.Message}");
            }
        }
        ...
        public void SetIntFieldValue(string value, string sField)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"SetIntFieldValue error: {ex.Message}");
            }
        }

        public void AvCoreDensity()
        {
            try
            {
                if (dr == null) return;
                ...
                if (!(dr["Core Density - IP 1"] == DBNull.Value))
                {
                    nCount += 1;
                    dSum += (double)dr["Core Density - IP 1"];
                }
                if (!(dr["Core Density - IP 2"] == DBNull.Value))
                {
                    nCount += 1;
                    dSum += (double)dr["Core Density - IP 2"];
                }
                if (!(dr["Core Density - IP 3"] == DBNull.Value))
                {
                    nCount += 1;
                    dSum += (double)dr["Core Density - IP 3"];
                }
                ...
                if (nCount > 0)
                {
                    ...
                    {
                        dr["Core Density - IP"] = dtmp;
                    }
                    else
                    {
                        dr["Core Density - IP"] = DBNull.Value;
                    }
                }
                else
                {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"AvCoreDensity error: {ex.Message}");
                dr["Core Density - IP"] = DBNull.Value;
            }
        }

        public void UpdateThicknessCalculations()
        {
            try
            {
                if (dr == null) return;
                ...
                if (nCount == 17 && !double.IsNaN(dSum / (double)nCount))
                {
                    dtmp = dSum / (double)nCount;
                    dr["Thickness - IP"] = dtmp;
                    dr["thickness IP- valleys"] = dmin;
                    dr["thickness peaks IP"] = dmax;
                    dr["Flatness IP"] = dmin - dmax;
                }
                else
                {
                    dr["Thickness - IP"] = DBNull.Value;
                    dr["thickness IP- valleys"] = DBNull.Value;
                    dr["thickness peaks IP"] = DBNull.Value;
                    dr["Flatness IP"] = DBNull.Value;
                }

                // Calculate Avg1 (positions 1-8)
                nCount = 0;
                dSum = 0.0;
                for (int i = 1; i < 9; i++)
                {
                ...
                if (nCount == 8 && !double.IsNaN(dSum / (double)nCount))
                {
                    dtmp = dSum / (double)nCount;
                    dr["Thickness IP Avg1"] = dtmp;
                }
                else
                {
                    dr["Thickness IP Avg1"] = DBNull.Value;
                }

                // Calculate Avg2 (positions 10-17)
                nCount = 0;
                dSum = 0.0;
                for (int i = 10; i < 18; i++)
                {
                ...
                {
                    dtmp = dSum / (double)nCount;
                    dr["Thickness IP Avg2"] = dtmp;
                }
                else
                {
                    dr["Thickness IP Avg2"] = DBNull.Value;
                }

                // Calculate slope
                if (dr["Thickness IP Avg1"] != DBNull.Value && dr["Thickness IP Avg2"] != DBNull.Value)
                {
                    dtmp = Math.Abs((double)dr["Thickness IP Avg1"] - (double)dr["Thickness IP Avg2"]);
                    dr["IP Thickness Slope"] = dtmp;
                }
                else
                {
                    dr["IP Thickness Slope"] = DBNull.Value;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"UpdateThicknessCalculations error: {ex.Message}");
            }
        }

        public void UpdateCompressiveStrengthCalculations()
        {
            try
            {
                if (dr == null) return;
                ...
                if (!(dr["Compressive IP - 1"] == DBNull.Value))
                {
                    nCount += 1;
                    dtmp = (double)dr["Compressive IP - 1"];
                    dSum += dtmp;
                    if (dMin > dtmp) dMin = dtmp;
                }
                if (!(dr["Compressive IP - 2"] == DBNull.Value))
                {
                    nCount += 1;
                    dtmp = (double)dr["Compressive IP - 2"];
                    dSum += dtmp;
                    if (dMin > dtmp) dMin = dtmp;
                }
                if (!(dr["Compressive IP - 3"] == DBNull.Value))
                {
                    nCount += 1;
                    dtmp = (double)dr["Compressive IP - 3"];
                    dSum += dtmp;
                    if (dMin > dtmp) dMin = dtmp;
                }
                if (!(dr["Compressive IP - 4"] == DBNull.Value))
                {
                    nCount += 1;
                    dtmp = (double)dr["Compressive IP - 4"];
                    dSum += dtmp;
                    if (dMin > dtmp) dMin = dtmp;
                }
                if (!(dr["Compressive IP - 5"] == DBNull.Value))
                {
                    nCount += 1;
                    dtmp = (double)dr["Compressive IP - 5"];
                    dSum += dtmp;
                    if (dMin > dtmp) dMin = dtmp;
                }
                if (!(dr["Compressive IP - 6"] == DBNull.Value))
                {
                    nCount += 1;
                    dtmp = (double)dr["Compressive IP - 6"];
                    dSum += dtmp;
                    if (dMin > dtmp) dMin = dtmp;
                }
                ...
                if (nCount == 6)
                {
                    dr["Compressive Strength - IP"] = dSum / 6.0;
                    dr["Compressive Strength 5 - IP"] = (dSum - dMin) / 5.0;
                }
                else if (nCount == 5)
                {
                    dr["Compressive Strength - IP"] = DBNull.Value;
                    dr["Compressive Strength 5 - IP"] = dSum / 5.0;
                }
                else
                {
                    dr["Compressive Strength - IP"] = DBNull.Value;
                    dr["Compressive Strength 5 - IP"] = DBNull.Value;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"UpdateCompressiveStrengthCalculations error: {ex.Message}");
            }
        }

        public void UpdateCoreDensityCalculation(int sampleNumber)
        {
            try
            {
                if (dr == null) return;

                string suffix = sampleNumber.ToString();
                int nCount = 0;
                double dSum = 0, dtmp;
                ...
                // Mass
                string massField = "Mass " + suffix;
                if (!(dr[massField] == DBNull.Value))
                {
                    dm = (double)dr[massField];
                    nm = 1;
                }

                // Length (L1 only)
                nCount = 0; dSum = 0;
                string l1Field = "L1 " + suffix;
                if (!(dr[l1Field] == DBNull.Value))
                {
                    nCount += 1;
                    dSum += (double)dr[l1Field];
                }
                if (nCount > 0)
                {
                    dl = dSum / (double)nCount;
                    nl = 1;
                }

                // Width (W1 only)
                nCount = 0; dSum = 0;
                string w1Field = "W1 " + suffix;
                if (!(dr[w1Field] == DBNull.Value))
                {
                    nCount += 1;
                    dSum += (double)dr[w1Field];
                }
                if (nCount > 0)
                {
                    dw = dSum / (double)nCount;
                    nw = 1;
                }
                ...
                for (int i = 1; i <= 5; i++)
                {
                    string tField = "T" + i + " " + suffix;
                    if (!(dr[tField] == DBNull.Value))
                    {
                        nCount += 1;
                        dSum += (double)dr[tField];
                    }
                }
                if (nCount > 3)
                {
                    dt = dSum / (double)nCount;
                    nt = 5;
                }

                string densityField = "Core Density - IP " + suffix;
                if (nm * nl * nw * nt > 0 && dl * dw * dt > 0.0)
                {
                    dtmp = dm / (dl * dw * dt) * 3.809590998;
                    dr[densityField] = dtmp;
                }
                else
                {
                    dr[densityField] = DBNull.Value;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"UpdateCoreDensityCalculation error for sample {sampleNumber}: {ex.Message}");
            }
        }
    }
}
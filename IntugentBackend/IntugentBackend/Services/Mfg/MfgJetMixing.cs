    ...
    public class MfgJetMixing
    {
        ...
        public void Startup()
        {
            try
            {
                ...
                if (cLists?.drEmployee != null &&
                    cLists.drEmployee.Table?.Columns.Contains("sJetMix") == true &&
                    cLists.drEmployee["sJetMix"] != DBNull.Value)
                {
                    string js1 = cLists.drEmployee["sJetMix"]?.ToString();
                    if (!string.IsNullOrWhiteSpace(js1))
                    {
                        try
                        {
                            var result = System.Text.Json.JsonSerializer.Deserialize(js1, typeof(CJetMix));
                            if (result != null)
                            {
                                JetMix1 = (CJetMix)result;
        ...
        public void SetDefaultValues()
        {
            try
            {
                double toSiPres = cUConv?.ToSi_Pres ?? 1.0;
                ...
                JetMix1.FRate_A = 0.33951;
                JetMix1.FRate_B = 0.24509;
                JetMix1.Temp_A = 298.15;
                JetMix1.Temp_B = 298.15;
                JetMix1.Pres_A = 2000.0 * toSiPres;
                JetMix1.Pres_B = 2000.0 * toSiPres;
                JetMix1.Dens_A = 1278;
                JetMix1.Dens_B = 1106;
                JetMix1.Vis_A = 0.70;
                JetMix1.Vis_B = 5.68;
                JetMix1.VisO_A = 0.70;
                JetMix1.VisO_B = 1.5;
                JetMix1.VisE_A = 2000.0;
                JetMix1.VisE_B = 2500;
                JetMix1.Pres_Max = 4000 * toSiPres;
                JetMix1.Pres_Min = 1000 * toSiPres;
                JetMix1.ReNo_Min = 100.0;
            }
            catch (Exception ex)
            {
